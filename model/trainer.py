...
class XMemTrainer:
    ...
    def do_pass(self, data, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            ...
            key, shrinkage, selection, f16, f8, f4, key_8, key_4 = self.XMem('encode_key', frames)

            filler_one = torch.zeros(1, dtype=torch.int64)
            hidden = torch.zeros((b, num_objects, self.config['hidden_dim'], *key.shape[-2:]))
            v16, v8_down, v4_down, hidden = self.XMem('encode_value', frames[:,0],
                                    f16[:,0], f8[:,0], f4[:,0], 
                                    hidden, first_frame_gt[:,0])
            values = v16.unsqueeze(3)
            
            values_8 = v8_down.unsqueeze(3)
            values_4 = v4_down.unsqueeze(3)
            
            for ti in range(1, self.num_frames):
                if ti <= self.num_ref_frames:
                    ref_values = values
                    ref_values_8 = values_8
                    ref_values_4 = values_4
                    ref_keys = key[:,:,:ti]
                    ref_keys_8 = key_8[:,:,:ti]
                    ref_keys_4 = key_4[:,:,:ti]
                    ...
                    ref_values_8 = torch.stack([
                        values_8[bi, :, :, indices[bi]] for bi in range(b)
                    ], 0)
                    ref_values_4 = torch.stack([
                        values_4[bi, :, :, indices[bi]] for bi in range(b)
                    ], 0)
                    ref_keys = torch.stack([
                        key[bi, :, indices[bi]] for bi in range(b)
                    ], 0)
                    ref_keys_8 = torch.stack([
                        key_8[bi, :, indices[bi]] for bi in range(b)
                    ], 0)
                    ref_keys_4 = torch.stack([
                        key_4[bi, :, indices[bi]] for bi in range(b)
                    ], 0)
                ...
                memory_readout = self.XMem('read_memory', key[:,:,ti], selection[:,:,ti] if selection is not None else None, 
                                        ref_keys, ref_shrinkage, ref_values, 1)
                memory_readout_8 = self.XMem('read_memory', key_8[:,:,ti], None, 
                                        ref_keys_8, None, ref_values_8, 2)
                memory_readout_4 = self.XMem('read_memory', key_4[:,:,ti], None, 
                                        ref_keys_4, None, ref_values_4, 4)
                ...
                if ti < (self.num_frames-1):
                    ...
                    v16, v8_down, v4_down, hidden = self.XMem('encode_value', frames[:,ti], 
                                                              f16[:,ti], f8[:,ti], f4[:,ti], 
                                                              hidden, masks, is_deep_update=is_deep_update)
                    values = torch.cat([values, v16.unsqueeze(3)], 3)
                    values_8 = torch.cat([values_8, v8_down.unsqueeze(3)], 3)
                    values_4 = torch.cat([values_4, v4_down.unsqueeze(3)], 3)
        ...
        if self.config['amp']:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(self.XMem.parameters(), 
                                                       3.0)
