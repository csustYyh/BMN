...
parser.add_argument('--model', default='') # model path

# Data options
parser.add_argument('--d16_path', default='')
parser.add_argument('--d17_path', default='')
parser.add_argument('--y18_path', default='')
parser.add_argument('--y19_path', default='')
# For generic (G) evaluation, point to a folder that contains "JPEGImages" and "Annotations"
parser.add_argument('--generic_path', default='')

parser.add_argument('--dataset', help='D16/D17/Y18/Y19/LV1/LV3/G', default='')
...
parser.add_argument('--size', default=600, type=int, 
...
is_g = args.dataset.startswith('G')
...
agent_pth_path = '' # agent path
agent_weights = torch.load(agent_pth_path)
agent = Agent(device='cuda')
agent.policy_net.load_state_dict(agent_weights)
agent.set_eval()
...
for vid_reader in progressbar(meta_loader, max_value=len(meta_dataset), redirect_stdout=True):
    ...
    mapper = MaskMapper()
    processor = InferenceCore(agent, network, config=config)
