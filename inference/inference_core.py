...
class InferenceCore:
    def __init__(self, agent, network:XMem, config):
        self.config = config
    ...
    def clear_memory(self):
        ...
        self.memory8 = MemoryManager(config=self.config)
        self.memory4 = MemoryManager(config=self.config)
    ...
    def update_config(self, config):
        ...
        self.memory8.update_config(config)
        self.memory4.update_config(config)
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        key, shrinkage, selection, f16, f8, f4, k8, k4, merge_8, merge_4, unmerge_8, unmerge_4 = self.network.encode_key(image, 
        ...
        if need_segment:
            ...
            t, denoise_memory = self.memory.get_denoise_memory_idx(key, self.agent)
            memory_readout = self.memory.match_memory(key, selection).unsqueeze(0)
            memory_readout_8 = self.memory8.match_memory_with_agent(t, denoise_memory, k8, None).unsqueeze(0)
            memory_readout_4 = self.memory4.match_memory_with_agent(t, denoise_memory, k4, None).unsqueeze(0)
            hidden, _, pred_prob_with_bg = self.network.segment(multi_scale_features, 
                                    memory_readout, memory_readout_8, memory_readout_4, unmerge_8, unmerge_4,
        ...
        if is_mem_frame:
            value, value8, value4, hidden = self.network.encode_value(image, f16, f8, f4, self.memory.get_hidden(), 
                                    pred_prob_with_bg[1:].unsqueeze(0), merge_8, merge_4, is_deep_update=is_deep_update)
            self.memory.add_memory(key, shrinkage, value, self.all_labels, 
                                    selection=selection if self.enable_long_term else None)
            self.memory8.add_memory(k8, shrinkage, value8, self.all_labels, 
                                    selection=selection if self.enable_long_term else None)
            self.memory4.add_memory(k4, shrinkage, value4, self.all_labels, 
                                    selection=selection if self.enable_long_term else None)
    ...
    def step_viz(self, image, mask=None, valid_labels=None, end=False):
        # image: 3*H*W
        # mask: num_objects*H*W or None
        self.curr_ti += 1
        image, self.pad = pad_divide_by(image, 16)
        ...
        if need_segment:
            memory_readout = self.memory.match_memory(key, selection).unsqueeze(0)
            memory_readout_8 = self.memory8.match_memory(k8, None).unsqueeze(0)
            memory_readout_4 = self.memory4.match_memory(k4, None).unsqueeze(0)
            hidden, dec_feature, pred_prob_with_bg, g4= self.network.segment(multi_scale_features, 
        ...
        if g4 is not None:
            g4 = F.interpolate(g4, scale_factor=4, mode='bilinear', align_corners=False)
            unpad(g4, self.pad)
            unpad(dec_feature, self.pad)
            
            
        return unpad(pred_prob_with_bg, self.pad), g4, dec_feature
