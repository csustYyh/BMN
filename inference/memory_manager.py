...
class MemoryManager:
    ...
    def get_denoise_memory_idx(self, q_k, agent):
        denoise_memory = [0] # first gt
        _, ck, h, w = q_k.shape
        m_k = self.work_mem.key.view(1, ck, h, w, -1)
        _, _, _, _, t = m_k.shape
        
        for it in range(1, t):
            state = torch.cat([q_k,m_k[:, :, :, :, it]],dim = 1)
            action = agent.rl_action(state, True)
            if action > 0: denoise_memory.append(it) # add in denoised memory
        return t, denoise_memory
    
    def get_denoise_memory(self, q_k, denoise_memory):
        _, ck, h, w = q_k.shape
        m_k = self.work_mem.key.view(1, ck, h, w, -1)
            
        # value
        m_v = self.work_mem.value   # [tensor, tensor,...]
        dm_v_tmp = []
        clip_start = []
        time_clip = []
        num_groups = self.work_mem.num_groups
        for gi in range(0, num_groups): 
            b, cv, _ = m_v[gi].shape
            reshaped_tensor = m_v[gi].view(b, cv, h, w, -1)
            dm_v_tmp.append(reshaped_tensor)
            if gi == 0:
                clip_start.append(0)
            if gi > 0:
                clip_start.append(self.work_mem.get_v_size(0)//(h*w) - self.work_mem.get_v_size(gi)//(h*w))
            time_clip.append([])
        denoise_memory += [item for item in clip_start if item not in denoise_memory]
        denoise_memory.sort()
        for ti in denoise_memory:
            for i, st in enumerate(clip_start):
                if ti >= st:
                    time_clip[i].append(ti - st)
        dm_v = []

        for gi in range(0, num_groups):
            dm_v.append(dm_v_tmp[gi][:, :, :, :, time_clip[gi]])
        dm_v = [tmp.flatten(start_dim=2) for tmp in dm_v]

        index = torch.tensor(denoise_memory).cuda()
        dm_k = m_k.index_select(dim=4, index=index)
        dm_k = dm_k.flatten(start_dim=2)
        
        # shrinkage
        m_s = self.work_mem.shrinkage.view(1, 1, h, w, -1)
        dm_s = m_s[:, :, :, :, denoise_memory] 
        dm_s = dm_s.flatten(start_dim=2)

        dm_group_num = []
        for it in time_clip:
            dm_group_num.append(len(it))
        return dm_k, dm_v, dm_s, dm_group_num
        
    def match_memory_with_agent(self, t, denoise_memory, query_key, selection):
        # query_key: B x C^k x H x W
        # selection:  B x C^k x H x W
        num_groups = self.work_mem.num_groups
        h, w = query_key.shape[-2:]

        self.dm_k, self.dm_v, self.dm_s, dm_n = self.get_denoise_memory(query_key, denoise_memory)
        ...
        if self.enable_long_term and self.long_mem.engaged():
            # Use long-term memory
            long_mem_size = self.long_mem.size
            memory_key = torch.cat([self.long_mem.key, self.dm_k], -1)
            shrinkage = torch.cat([self.long_mem.shrinkage, self.dm_s], -1) 
            ...
            for gi in range(1, num_groups):
                if gi < self.long_mem.num_groups:
                    ...
                    affinity_one_group = do_softmax(
                        torch.cat([long_mem_similarity[:, -self.long_mem.get_v_size(gi):], 
                                    work_mem_similarity[:, -dm_n[gi]*h*w:]], 1), 
                        top_k=self.top_k, inplace=True)
                else:
                    # no long-term memory for this group
                    affinity_one_group = do_softmax(work_mem_similarity[:, -dm_n[gi]*h*w:], 
                        top_k=self.top_k, inplace=(gi==num_groups-1))
                affinity.append(affinity_one_group)

            all_memory_value = []
            for gi, gv in enumerate(self.dm_v):
                # merge the working and lt values before readout
                if gi < self.long_mem.num_groups:
                    all_memory_value.append(torch.cat([self.long_mem.value[gi], self.dm_v[gi]], -1))
                else:
                    all_memory_value.append(gv)
            ...
            # ignore the index return for long-term memory
            work_usage = usage[:, long_mem_size:]
            work_pad_usage = torch.zeros(1, t*h*w, dtype=work_usage.dtype, device=work_usage.device)
            chunks = work_usage.split(h*w, dim=1)
            for i, pos in enumerate(denoise_memory):
                work_pad_usage[:, pos*h*w : (pos+1)*h*w] = chunks[i]
            self.work_mem.update_usage(work_pad_usage.flatten())
            ...
            # No long-term memory
            similarity = get_similarity(self.dm_k, self.dm_s, query_key, selection)
            ...
            if self.enable_long_term:
                ...
                # Record memory usage for working memory
                pad_usage = torch.zeros(1, t*h*w, dtype=usage.dtype, device=usage.device)
                chunks = usage.split(h*w, dim=1)
                for i, pos in enumerate(denoise_memory):
                    pad_usage[:, pos*h*w : (pos+1)*h*w] = chunks[i]
                self.work_mem.update_usage(pad_usage.flatten())
            else:
                affinity = do_softmax(similarity, inplace=(num_groups==1), 
                    top_k=self.top_k, return_usage=False)
            ...
            # compute affinity group by group as later groups only have a subset of keys
            for gi in range(1, num_groups):
                affinity_one_group = do_softmax(similarity[:, -dm_n[gi]*h*w:], 
                    top_k=self.top_k, inplace=(gi==num_groups-1))
                affinity.append(affinity_one_group)
                affinity_one_group

                
            all_memory_value = self.dm_v
